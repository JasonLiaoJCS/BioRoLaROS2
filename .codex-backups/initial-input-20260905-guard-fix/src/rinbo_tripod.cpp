    ...
    PIDController() : Node("rinbo_tripod_rslip") {
        ...
        if (disabled_legs_.enabled()) {
            RCLCPP_WARN(this->get_logger(),
                        "DEGRADED MODE: hardware.disabled_legs=[%s]. Disabled main drives are forced to zero; per-servo disable is unavailable, so physical servo isolation is required.",
...
private:
    ...
    struct SafetyConfig {
        ...
        float max_bus_voltage = 30.0f;
        float max_current = 3.0f;
        float max_bus_current = 30.0f;
        float max_position_error_counts = 5000.0f;
    ...
    void load_safety_parameters() {
        ...
        safety_.max_bus_voltage = static_cast<float>(this->declare_parameter<double>("safety.max_bus_voltage", 30.0));
        safety_.max_current = static_cast<float>(this->declare_parameter<double>("safety.max_current", 3.0));
        safety_.max_bus_current = static_cast<float>(this->declare_parameter<double>("safety.max_bus_current", 30.0));
        safety_.max_position_error_counts = static_cast<float>(this->declare_parameter<double>("safety.max_position_error_counts", 5000.0));
        ...
        safety_.current_trip_samples = this->declare_parameter<int>("safety.current_trip_samples", 10);
    ...
    bool trigger_safety_stop(const std::string& reason) {
        if (state_ != State::SAFETY_STOP) {
            ...
            RCLCPP_ERROR(this->get_logger(), "=== SAFETY STOP: %s ===", safety_stop_reason_.c_str());
    ...
    bool check_position_error_safety(const std::array<float, 6>& pos_errors) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            position_error_trip_counts_[i] = error > safety_.max_position_error_counts
                ? position_error_trip_counts_[i] + 1 : 0;
        ...
        if (worst_idx >= 0) {
            ...
            reason << "position error " << leg_name(worst_idx) << "=" << worst_abs_error
                   ...
                   << " errors=" << format_float_array(pos_errors);
    ...
    void state_callback(
        ...
        if (check_position_error_safety(pos_errors)) {
            update_previous_samples(positions, now_time);
            return;
        }
        ...
        enforce_disabled_leg_commands(cmd);
        set_servos(cmd, *msg);
    ...
    void set_servo_hold_targets(
        ...
        const auto target = [&](int index) {
            // A disconnected encoder can report UINT32 sentinels.  Disabled
            // servos are physically isolated, so use the calibrated neutral
            // rather than forwarding untrusted readback to the final arbiter.
    ...
    void force_leg_disabled(LegCommand& leg) {
        leg.enable = false;
        leg.direction = false;
        leg.voltage = 0.0f;
        leg.state = 0;
    ...
    void enforce_disabled_leg_commands(rinbo_msgs::msg::MotorCmdStamped& cmd) {
        if (disabled_legs_.contains(0)) force_leg_disabled(cmd.l1);
        if (disabled_legs_.contains(1)) force_leg_disabled(cmd.l2);
        if (disabled_legs_.contains(2)) force_leg_disabled(cmd.l3);
        if (disabled_legs_.contains(3)) force_leg_disabled(cmd.r1);
        if (disabled_legs_.contains(4)) force_leg_disabled(cmd.r2);
        if (disabled_legs_.contains(5)) force_leg_disabled(cmd.r3);
    }
