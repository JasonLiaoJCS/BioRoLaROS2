...
double motor_command_max_pwm = 80.0;
...
constexpr double kHardMotorCommandMaxPwm = 80.0;
...
std::optional<std::string> validate_motor_command_payload(
    ...
    for (std::size_t index = 0; index < legs.size(); ++index) {
        ...
        if (!std::isfinite(voltage)) {
            return std::string("non-finite main voltage for ") + leg_names[index];
        }
        if (voltage < 0.0 || voltage > motor_command_max_pwm) {
            std::ostringstream out;
            out << "main voltage for " << leg_names[index] << " is " << voltage
                << ", outside [0," << motor_command_max_pwm << "]";
            return out.str();
        }
    }
...
int main(int argc, char **argv) {
    ...
    motor_command_max_pwm =
        node->declare_parameter<double>("motor_command_max_pwm", 80.0);
    ...
    if (invalid_parameters) {
        RCLCPP_FATAL(
            ...
            "power max-age must be <=200ms; max-PWM must be finite and in (0,80]; servo encoder "
