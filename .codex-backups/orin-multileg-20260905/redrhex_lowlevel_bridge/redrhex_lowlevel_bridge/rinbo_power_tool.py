...
def _healthy_leg_current_channels(disabled_leg: str | None) -> tuple[int, ...]:
    disabled_channel = LEG_CURRENT_CHANNEL_BY_NAME.get(
        str(disabled_leg or "").strip().upper()
    )
    return tuple(
        channel for channel in LEG_CURRENT_CHANNELS if channel != disabled_channel
    )


def _relay_feedback_violation(msg, disabled_leg: str | None = None) -> str | None:
    ...
    healthy_current_channels = _healthy_leg_current_channels(disabled_leg)
    healthy_currents = [
    ...
    if max_current > HARD_MAX_HEALTHY_LEG_CURRENT_A:
        return (
            f"healthy-leg current {max_current:.3f}A is above "
...
class RinboPowerTool(Node):
    ...
    def summarize_power_state(self, msg) -> dict:
        ...
        summary["disabled_leg"] = self.args.disabled_leg or "none"
                ...
                if receipt != last_processed_receipt:
                    ...
                    if bool(self.last_power_state.power):
                        last_violation = _relay_feedback_violation(
                            self.last_power_state, self.args.disabled_leg
                        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--disabled-leg",
        choices=tuple(LEG_CURRENT_CHANNEL_BY_NAME),
        default=None,
        help=(
            "One predeclared, physically isolated leg whose own current channel "
            "is excluded from the relay-on current gate. All other legs remain protected."
        ),
    )
    return parser
