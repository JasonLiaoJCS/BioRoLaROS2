...
def test_relay_power_guard_keeps_healthy_legs_and_excludes_only_declared_leg() -> None:
    ...
    assert "current" in str(_relay_feedback_violation(state, None))
...
def test_interrupted_power_command_exits_nonzero_and_keeps_state_unknown(monkeypatch) -> None:
    instances = []
...
def test_interrupted_status_does_not_publish_power_command(monkeypatch) -> None:
    instances = []
