...
public:
    explicit StandingController(rinbo_config::MotionSession& session)
        ...
        rinbo_fsm::validate_bounded_timeout(
            hall_search_timeout_s_, 60.0, "safety.hall_search_timeout_s");
        rinbo_fsm::validate_bounded_timeout(
            rotate_timeout_s_, 60.0, "safety.rotate_timeout_s");
            ...
            switch (leg_states_[i]) {
                case LegState::FIND_HALL: {
                    if (!hall_effects[i]) {
                        ...
                        float vel_error = target_vel - control_velocity_[i];
                ...
                case LegState::ROTATE_180: {
                    ...
                    const bool settled = rotate_elapsed >= rotate_duration &&
                        final_error < position_tolerance_ && std::fabs(velocities[i]) < 500.0f;
                    if (!settled) stable_since_s_[i] = -1.0;
                    else if (stable_since_s_[i] < 0.0) stable_since_s_[i] = now_time.seconds();
                    if (settled && now_time.seconds() - stable_since_s_[i] >= 0.3) {
                        leg_states_[i] = LegState::DONE;
                        ++done_count;
                        RCLCPP_INFO(this->get_logger(), "%s: DONE! stationary for 0.3s, pos: %.0f, err: %.0f",
                ...
                case LegState::DONE: {
                    ...
                    if (std::fabs(pos_error) > rinbo_fsm::kHardMaxPositionErrorCounts) {
                    ...
                    float pwm = 0.1f * pos_error - kd_ * control_velocity_[i];
                    pwm = std::clamp(pwm, -max_pwm_, max_pwm_);
                    set_leg_cmd(cmd, i, pwm);
                    done_count++;
                    break;
