...
日常操作統一看 [Sim2Real 手冊](../../docs/redrhex_sim2real_sbrio.md)：新模型分類、校正、封裝、上機前檢查、R-Slip 接續與停止都在同一頁。

- [三大工作入口](../../docs/README.md)：單腳測試、Sim2Real、腿部管理。
- [腿部啟用／屏蔽](../../docs/orin_multileg_operation.md)：目前 FSM 與電源工具的共用名單；RL 模型仍有獨立的腿部規格，不能直接套用任意多腿組合。

手冊主流程是 L1 已實體隔離的架空 `full_feedback_rig` 範本，不能把範本當成當下硬體狀態。名單與校正以現場設定及量測為準。

本 README 說明程式規格；不重複上電操作步驟。
...
`policy_onnx_runner.py` 已有 sensor-v2 雙輸入／雙輸出支援，`observation_builder.py` 有 60-frame 資料組裝，控制節點也有對應分支。但 `scripts/package_verified_policy.py` 仍明確拒絕 `redrhex.sensor-policy-bundle.v2`，不能以 runtime 能讀取就宣稱完整部署驗證已完成。

sensor-v2 先使用 `check_onnx_io.py --contract experimental-sensor-v2` 搭配可信 SHA256 與 sidecar 做離線檢查。56／280 維封裝流程、metadata 與上機許可不能直接套用；詳細位置與下一步見主手冊。
