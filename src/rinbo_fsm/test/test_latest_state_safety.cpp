...
TEST(BridgeInputPreparationTest, UnknownAttributionWaitsButWrongOrDuplicateSourcesFail) {
    using rinbo_fsm::bridge_input_discovery;
    using State = rinbo_fsm::BridgeInputDiscovery;
    EXPECT_EQ(bridge_input_discovery(0, "", "", "/motor/state"), State::Pending);
    EXPECT_EQ(bridge_input_discovery(1, "_NODE_NAME_UNKNOWN_", "_NODE_NAMESPACE_UNKNOWN_", "/motor/state"), State::Pending);
    EXPECT_EQ(bridge_input_discovery(1, "_NODE_NAME_UNKNOWN_", "/", "/motor/state"), State::Pending);
    EXPECT_EQ(bridge_input_discovery(1, "rinbo_ros2_bridge", "_NODE_NAMESPACE_UNKNOWN_", "/motor/state"), State::Pending);
    EXPECT_EQ(bridge_input_discovery(1, "rinbo_ros2_bridge", "/", "/motor/state"), State::Ready);
    EXPECT_THROW(bridge_input_discovery(2, "rinbo_ros2_bridge", "/", "/motor/state"), std::runtime_error);
    EXPECT_THROW(bridge_input_discovery(1, "other", "_NODE_NAMESPACE_UNKNOWN_", "/motor/state"), std::runtime_error);
    EXPECT_THROW(bridge_input_discovery(1, "_NODE_NAME_UNKNOWN_", "/wrong", "/motor/state"), std::runtime_error);
}

TEST_F(RosSafetyTest, PreparationWaitsBeyondTwoSecondsWithoutStartingCommandPublisherOrInputClock) {
    auto node = std::make_shared<rclcpp::Node>("delayed_bridge_preparation_test");
    std::shared_ptr<rclcpp::Node> bridge;
    rclcpp::Publisher<rinbo_msgs::msg::MotorStateStamped>::SharedPtr motor;
    rclcpp::Publisher<rinbo_msgs::msg::PowerStateStamped>::SharedPtr power;
    std::exception_ptr creation_error;
    std::thread creator([&] {
        try {
            std::this_thread::sleep_for(2400ms); // reproduce the field's >2 s discovery
            bridge = std::make_shared<rclcpp::Node>("rinbo_ros2_bridge");
            motor = bridge->create_publisher<rinbo_msgs::msg::MotorStateStamped>("/motor/state", 1);
            power = bridge->create_publisher<rinbo_msgs::msg::PowerStateStamped>("/power/state", 1);
        } catch (...) { creation_error = std::current_exception(); }
    });
    std::exception_ptr preparation_error;
    bool saw_no_commands = false;
    try {
        rinbo_fsm::wait_for_bridge_inputs(*node, [&] {
            EXPECT_TRUE(node->get_publishers_info_by_topic("/motor/command").empty());
            saw_no_commands = true;
            return false;
        });
    } catch (...) { preparation_error = std::current_exception(); }
    creator.join();
    ASSERT_FALSE(creation_error);
    ASSERT_FALSE(preparation_error);
    ASSERT_TRUE(saw_no_commands);
    // Construct the normal guard only after preparation. No GID/header was
    // trusted by the preparation phase; a forged first callback still fails.
    rinbo_fsm::RosInputGuard guard(*node, "/motor/state");
    rinbo_msgs::msg::Header header;
    header.seq = 1;
    header.stamp = node->now();
    EXPECT_TRUE(guard.accept(header, message_info_with_gid(PublisherGid {})).violation);
    header.stamp = node->now();
    const auto accepted = guard.accept(header, message_info_with_gid(sole_publisher_gid(*node, "/motor/state")));
    EXPECT_FALSE(accepted.violation);
    EXPECT_FALSE(accepted.pending_startup);
    EXPECT_DOUBLE_EQ(rinbo_fsm::kHardMaxMotorStateRequiredAfterS, 2.0);
    EXPECT_DOUBLE_EQ(rinbo_fsm::kHardMaxMotorStateStaleS, 0.25);
}

TEST_F(RosSafetyTest, PreparationTimeoutAndCancellationNeverCreateCommandPublisher) {
    auto node = std::make_shared<rclcpp::Node>("bounded_bridge_preparation_test");
    ASSERT_TRUE(wait_until([&] {
        return node->get_publishers_info_by_topic("/motor/state").empty() &&
               node->get_publishers_info_by_topic("/power/state").empty();
    }, 2s));
    EXPECT_THROW(rinbo_fsm::wait_for_bridge_inputs(*node, [] { return false; }, 50ms), std::runtime_error);
    EXPECT_THROW(rinbo_fsm::wait_for_bridge_inputs(*node, [] { return true; }), std::runtime_error);
    EXPECT_THROW(rinbo_fsm::wait_for_bridge_inputs(*node, [] { return false; }, 9s), std::invalid_argument);
    EXPECT_TRUE(node->get_publishers_info_by_topic("/motor/command").empty());
}

TEST(FsmSourceContractTest, ProductionDiscoveryPrecedesCommandPublishersAndGuardClocks) {
    const std::filesystem::path root(RINBO_FSM_SOURCE_DIR);
    for (const char* file : {"rinbo_cali.cpp", "rinbo_standing.cpp", "rinbo_tripod.cpp", "rinbo_manual.cpp"}) {
        const auto source = read_file(root / "src" / file);
        const auto discovery = source.find("rinbo_fsm::wait_for_bridge_inputs(*this");
        ASSERT_NE(discovery, std::string::npos) << file;
        EXPECT_LT(discovery, source.find("create_publisher<")) << file;
        EXPECT_LT(discovery, source.find("std::make_unique<rinbo_fsm::RosInputGuard>")) << file;
        EXPECT_LT(discovery, source.find("std::make_unique<rinbo_fsm::MotorArbiterHandshake>")) << file;
    }
}

TEST(InputPublisherStartupTest, OnlyZeroOrFullyUnknownCorrelatedEndpointsWait) {
    using rinbo_fsm::input_publisher_discovery_pending;
    EXPECT_TRUE(input_publisher_discovery_pending(0U, "", "", false, false, 0.05));
    EXPECT_TRUE(input_publisher_discovery_pending(
        1U, "_NODE_NAME_UNKNOWN_", "_NODE_NAMESPACE_UNKNOWN_", true, false, 0.05));
    EXPECT_TRUE(input_publisher_discovery_pending(
        1U, "_NODE_NAME_UNKNOWN_", "/_NODE_NAMESPACE_UNKNOWN_", true, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "_NODE_NAME_UNKNOWN_", "_NODE_NAMESPACE_UNKNOWN_", false, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "_NODE_NAME_UNKNOWN_", "/", true, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "rinbo_ros2_bridge", "_NODE_NAMESPACE_UNKNOWN_", true, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "not_the_bridge", "/", true, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        2U, "_NODE_NAME_UNKNOWN_", "_NODE_NAMESPACE_UNKNOWN_", true, false, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "rinbo_ros2_bridge", "/", true, false, 0.15));
}

TEST(InputPublisherStartupTest, WaitIsBoundedAndNeverReopensAfterTrust) {
    using rinbo_fsm::input_publisher_discovery_pending;
    EXPECT_TRUE(input_publisher_discovery_pending(0U, "", "", false, false, 1.999));
    EXPECT_FALSE(input_publisher_discovery_pending(0U, "", "", false, false, 2.0));
    EXPECT_FALSE(input_publisher_discovery_pending(0U, "", "", false, false, -0.01));
    EXPECT_FALSE(input_publisher_discovery_pending(
        0U, "", "", false, false, std::numeric_limits<double>::quiet_NaN()));
    EXPECT_FALSE(input_publisher_discovery_pending(0U, "", "", false, true, 0.05));
    EXPECT_FALSE(input_publisher_discovery_pending(
        1U, "_NODE_NAME_UNKNOWN_", "_NODE_NAMESPACE_UNKNOWN_", true, true, 0.05));
    const rinbo_fsm::RosInputGuard::Validation pending {
        10000000000LL, 10000000000LL, std::nullopt, true};
    EXPECT_FALSE(pending.fresh_at_validation(0.25));
}

TEST_F(RosSafetyTest, PendingInitialCallbackDoesNotConsumeHeaderOrPinPublisher) {
    auto node = std::make_shared<rclcpp::Node>("guard_pending_startup_test");
    const std::string topic = "/rinbo_fsm_safety_test/guard_motor_initial_pending";
    rinbo_fsm::RosInputGuard guard(*node, topic);
    rinbo_msgs::msg::Header header;
    header.seq = 10U;
    header.stamp = node->now();
    const auto pending = guard.accept(header, message_info_with_gid(PublisherGid {}));
    ASSERT_TRUE(pending.pending_startup);
    EXPECT_FALSE(pending.violation.has_value());
    EXPECT_FALSE(pending.fresh_at_validation(0.25));
    ...
    // Same sequence as the discarded callback remains valid; use fresh data,
    // not the held startup sample, when endpoint attribution is complete.
    header.stamp = node->now();
    const auto trusted = guard.accept(
        header, message_info_with_gid(sole_publisher_gid(*node, topic)));
    EXPECT_FALSE(trusted.pending_startup);
    EXPECT_FALSE(trusted.violation.has_value())
        << (trusted.violation ? *trusted.violation : std::string());
    EXPECT_TRUE(trusted.fresh_at_validation(0.25));
    (void)publisher;
}

TEST_F(RosSafetyTest, PastStaleStartupSamplesWaitButFreshInputCommitsRuntimeChecks) {
    auto publisher_node = std::make_shared<rclcpp::Node>("rinbo_ros2_bridge");
    auto node = std::make_shared<rclcpp::Node>("guard_stale_startup_test");
    const std::string topic = "/rinbo_fsm_safety_test/guard_motor_stale_startup";
    auto publisher = publisher_node->create_publisher<
        rinbo_msgs::msg::MotorStateStamped>(topic, 1);
    ASSERT_TRUE(wait_until([&]() {
        const auto endpoints = node->get_publishers_info_by_topic(topic);
    ...
    const auto info = message_info_with_gid(sole_publisher_gid(*node, topic));
    ...
    hooks.steady_now = [&]() { return steady_now; };
    rinbo_fsm::RosInputGuard guard(*node, topic, std::move(hooks));
    rinbo_msgs::msg::Header header;
    header.seq = 10U;
    set_stamp(header, now_ns - 246961000LL);
    const auto stale = guard.accept(header, info);
    EXPECT_TRUE(stale.pending_startup);
    EXPECT_FALSE(stale.violation.has_value());
    EXPECT_FALSE(stale.fresh_at_validation(0.25));
    steady_now += 1500ms;
    EXPECT_TRUE(guard.accept(header, info).pending_startup);

    set_stamp(header, now_ns);
    const auto fresh = guard.accept(header, info);
    EXPECT_FALSE(fresh.pending_startup);
    ASSERT_FALSE(fresh.violation.has_value());
    EXPECT_TRUE(fresh.fresh_at_validation(0.25));
    header.seq = 11U;
    set_stamp(header, now_ns - 246961000LL);
    const auto runtime_stale = guard.accept(header, info);
    EXPECT_FALSE(runtime_stale.pending_startup);
    expect_rejection_contains(runtime_stale.violation, "source stamp age");
    (void)publisher;
}

TEST_F(RosSafetyTest, StartupBudgetDoesNotResetOnStaleSamples) {
    auto publisher_node = std::make_shared<rclcpp::Node>("rinbo_ros2_bridge");
    auto node = std::make_shared<rclcpp::Node>("guard_stale_startup_expiry_test");
    const std::string topic = "/rinbo_fsm_safety_test/guard_motor_stale_expiry";
    auto publisher = publisher_node->create_publisher<
        rinbo_msgs::msg::MotorStateStamped>(topic, 1);
    ASSERT_TRUE(wait_until([&]() {
        const auto endpoints = node->get_publishers_info_by_topic(topic);
    ...
    auto steady_now = std::chrono::steady_clock::time_point {};
    rinbo_fsm::RosInputGuard::TestHooks hooks;
    hooks.now_ns = []() { return 10000000000LL; };
    hooks.steady_now = [&]() { return steady_now; };
    rinbo_fsm::RosInputGuard guard(*node, topic, std::move(hooks));
    rinbo_msgs::msg::Header header;
    header.seq = 1U;
    set_stamp(header, 9700000000LL);
    EXPECT_TRUE(guard.accept(header, info).pending_startup);
    steady_now += 1999ms;
    EXPECT_TRUE(guard.accept(header, info).pending_startup);
    steady_now += 1ms;
    const auto expired = guard.accept(header, info);
    EXPECT_FALSE(expired.pending_startup);
    expect_rejection_contains(expired.violation, "source stamp age");
    (void)publisher;
}
...
TEST_F(RosSafetyTest, GraphDelayCannotRescueHeaderAlreadyStaleAtEntry) {
    ...
    rinbo_msgs::msg::Header header;
    header.seq = 1U;
    set_stamp(header, fake_now_ns);
    ASSERT_FALSE(guard.accept(header, message_info).violation.has_value());
    header.seq = 2U;
    set_stamp(header, fake_now_ns - 101000000LL);
...
TEST_F(RosSafetyTest, InvalidFutureReplaySequenceAndStampRemainRejected) {
    ...
    // First fresh input commits the runtime age/replay rules. Past-stale
    // samples before this point are startup drops, not accepted state.
...
TEST_F(RosSafetyTest, DisabledLegsAcceptMultipleCanonicalLegsAndRejectInvalidSets) {
    ...
    const auto multiple = rinbo_fsm::DisabledLegs::load(*too_many_node);
    EXPECT_EQ(multiple.count(), 2);
    EXPECT_TRUE(multiple.contains(0));
    EXPECT_TRUE(multiple.contains(4));
...
TEST_F(RosSafetyTest, PublishBoundaryMaskDisablesEachConfiguredMainDrive) {
    ...
    for (unsigned bits = 0; bits < 64; ++bits) {
        ...
        const auto disabled_legs = rinbo_fsm::DisabledLegs::from_names(names);
        EXPECT_EQ(disabled_legs.healthy_count(), 6 - static_cast<int>(names.size()));
        EXPECT_FALSE(disabled_legs.healthy_complete(-1));
        EXPECT_EQ(disabled_legs.healthy_complete(disabled_legs.healthy_count()), bits != 63);
        EXPECT_EQ(disabled_legs.all_healthy([](int) { return true; }), bits != 63);
        for (int missing = 0; missing < 6; ++missing) {
            EXPECT_EQ(disabled_legs.all_healthy([missing](int i) { return i != missing; }),
                      bits != 63 && disabled_legs.contains(missing));
        }
...
TEST_F(RosSafetyTest, DisabledLegParametersAreStartupOnlyAndRejectRuntimeUpdates) {
    ...
    EXPECT_EQ(node->get_parameter("hardware.max_disabled_legs").as_int(), 6);
...
TEST(FsmSourceContractTest, AllFsmsShareLatestQosAndDisabledLegSafetyContract) {
    ...
    for (const auto& file : files) {
        ...
        EXPECT_EQ(
            ...
            2U) << file;
        EXPECT_EQ(
            occurrence_count(compact, "if(input.pending_startup)return;"),
            2U) << file;
        EXPECT_NE(
        ...
        EXPECT_EQ(
            ...
            1U) << file;
        EXPECT_NE(source.find("supported_leg_test"), std::string::npos) << file;
        EXPECT_NE(source.find("rinbo_config::MotionSession"), std::string::npos) << file;
        EXPECT_NE(source.find("rinbo_config::check_config_cli"), std::string::npos) << file;
    }
    ...
    EXPECT_EQ(occurrence_count(tripod, "cmd_pub_->publish(cmd)"), 1U);
    EXPECT_EQ(occurrence_count(tripod, "force_leg_disabled(cmd."), 0U);
...
TEST(TripodPowerPolicyTest, ArmedOutputUsesDebounceAndHardCurrentStillStopsImmediately) {
    rinbo_fsm::TripodPowerPolicyConfig config;
    config.current_trip_samples = 3;
    rinbo_fsm::TripodPowerPolicy policy(config);
    policy.observe({1000000000LL, true, true, 36.0, 0.0, 0.0});
    ASSERT_TRUE(policy.ready_for_output(1000000000LL));
    policy.observe({1010000000LL, true, true, 36.0, 6.0, 0.0});
    EXPECT_TRUE(policy.ready_for_output(1010000000LL));
    policy.observe({1020000000LL, true, true, 36.0, 6.0, 0.0});
    EXPECT_TRUE(policy.ready_for_output(1020000000LL));
    policy.observe({1030000000LL, true, true, 36.0, 6.0, 0.0});
    EXPECT_FALSE(policy.ready_for_output(1030000000LL));
    policy.observe({1040000000LL, true, true, 36.0, 0.0, 0.0});
    EXPECT_TRUE(policy.ready_for_output(1040000000LL));
    policy.observe({1050000000LL, true, true, 36.0, 10.1, 0.0});
    EXPECT_FALSE(policy.ready_for_output(1050000000LL));
}
