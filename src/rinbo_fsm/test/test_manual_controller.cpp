...
struct ManualOfflineTestAccess {
    ...
    static std::string reason(const ManualController& node) { return node.reason_; }
...
TEST_F(ManualControllerTest, UsesAuthoritativeStandingEffortProfile) {
    const rinbo_fsm::MotionEffort tuned{.08,.006,.005,40,153.6};
    ASSERT_TRUE(rinbo_config::update_motion_effort(path,tuned));
    { rinbo_config::MotionSession c(rinbo_config::Stage::Calibration,path); c.complete(); }
    rinbo_config::MotionSession session(rinbo_config::Stage::Manual,path);
    ManualController node(session,plan());
    const auto actual=ManualOfflineTestAccess::effort(node);
    EXPECT_DOUBLE_EQ(actual.kp,tuned.kp);
    EXPECT_DOUBLE_EQ(actual.kd,tuned.kd);
    EXPECT_DOUBLE_EQ(actual.k_ff,tuned.k_ff);
    EXPECT_DOUBLE_EQ(actual.friction_pwm,tuned.friction_pwm);
    EXPECT_DOUBLE_EQ(actual.friction_velocity_counts_s,tuned.friction_velocity_counts_s);
}
...
enum class Fault { None, MotorStale, PowerStale, Estop, Epoch, Heartbeat, Nan, Overspeed, Stuck, StuckVelocity };
...
TEST_P(ManualWireTest, RealRosHandshakeMotionAndFailClosedOutput) {
    ...
    if (GetParam() == Fault::StuckVelocity) {
        p.legs[1].mode = rinbo_manual::Mode::Velocity;
        p.legs[1].speed = 5.0;
    }
    ...
    auto reports = bridge->create_subscription<std_msgs::msg::String>("/rinbo/manual/status",1,
        [&](std_msgs::msg::String::SharedPtr msg) { last_report = msg->data; });
    ...
    while (!node->finished() && seconds(Clock::now(),start) < 9.0) {
        if (Clock::now() >= next) {
            ...
            if (fault != Fault::MotorStale) {
                ...
                const auto q = (fault == Fault::Stuck || fault == Fault::StuckVelocity)
                    ? std::array<double,6>{} : ManualOfflineTestAccess::ideal_feedback(*node);
                m.l2.position = -q[1]*rinbo_manual::counts_per_degree;
                if (fault == Fault::Overspeed) m.l2.position -= 20000;
    ...
    if (GetParam() == Fault::Stuck || GetParam() == Fault::StuckVelocity) {
        const auto reason = ManualOfflineTestAccess::reason(*node);
        EXPECT_NE(reason.find(GetParam() == Fault::StuckVelocity
            ? "final pose not reached: L2" : "alignment not reached: L2"),std::string::npos);
        EXPECT_NE(reason.find("actual_deg=0.00"),std::string::npos);
        EXPECT_NE(reason.find("cap=20.00 encoder_span_deg=0.00 peak_pwm=20.00"),std::string::npos);
        EXPECT_NE(last_report.find("實際="),std::string::npos);
        EXPECT_NE(last_report.find("目標="),std::string::npos);
        EXPECT_NE(last_report.find("PWM命令=20.0/20.0"),std::string::npos);
    }
}
