...
struct TripodOfflineTestAccess {
    ...
    static void setup_ratio59(PIDController& f) {
        boundary(f);
        f.current_ratio_=5.9; f.target_ratio_=5.9;
        f.tau_=15*f.period_+f.t_center_; f.group_b_started_=true;
    }
    static void reproduce_delayed_reversal(PIDController& f) {
        // Actual logged R1 correction at ratio 5.898, with positive prior output.
        f.prev_command_pwms_.fill(134.5653f);
        std::array<float,6> command;command.fill(-725.5146f);
        f.apply_pwm_slew_limit(command,.001);
        for(int i:{0,1,3,4,5}) EXPECT_FLOAT_EQ(command[i],-725.5146f);
...
TEST_F(TripodMultiLegTest, Configured3300CanBeReachedWhileSlewMaskAndNormalStopRemainEffective) {
    warning_fixture();
    auto d=YAML::LoadFile(path);d["parameters"]["rinbo_tripod_rslip"]["max_pwm"]=3300.0;
    {std::ofstream out(path);out<<d;}
    {rinbo_config::MotionSession c(rinbo_config::Stage::Calibration,path);c.complete();}
    {rinbo_config::MotionSession stand(rinbo_config::Stage::Standing,path);stand.complete();}
    rinbo_config::MotionSession session(rinbo_config::Stage::Tripod,path);
    PIDController f(session);Access::initialize(f);
    std::array<float,6> positions{};std::array<float,6> previous{};
    bool exceeded80=false;
    for(int step=1;step<=2900;++step) {
        Access::tick(f,10+step*.01,positions);ASSERT_FALSE(Access::stopped(f));
        const auto debug=Access::debug(f);
        for(int leg=0;leg<6;++leg) {
            EXPECT_LE(std::fabs(debug.limited_pwm[leg]),3300);
            EXPECT_LE(std::fabs(debug.limited_pwm[leg]-previous[leg]),2.501);
            previous[leg]=debug.limited_pwm[leg];
            if(leg==2)EXPECT_EQ(debug.limited_pwm[leg],0);
        }
        exceeded80=exceeded80 || debug.limited_pwm[1]>80;
    }
    EXPECT_TRUE(exceeded80);EXPECT_FLOAT_EQ(previous[1],3300);
    Access::begin_stop(f);Access::tick(f,41.1,positions);
    EXPECT_TRUE(Access::fully_stopped(f));EXPECT_FALSE(Access::stopped(f));
}

TEST_F(TripodMultiLegTest, OptionalSlewOffPassesRatio59CorrectionsWithCapMaskAndStop) {
    warning_fixture();
    auto document=YAML::LoadFile(path);
    auto parameters=document["parameters"]["rinbo_tripod_rslip"];
    parameters["max_pwm"]=3300.0;
    parameters["safety"]["enable_pwm_slew_limit"]=false;
    {std::ofstream out(path);out<<document;}
    {rinbo_config::MotionSession c(rinbo_config::Stage::Calibration,path);c.complete();}
    {rinbo_config::MotionSession s(rinbo_config::Stage::Standing,path);s.complete();}
    rinbo_config::MotionSession session(rinbo_config::Stage::Tripod,path);PIDController f(session);
    Access::reproduce_delayed_reversal(f);
    Access::setup_ratio59(f);
    bool clipped=false,negative=false,positive=false;
    for(int n=0;n<500;++n) {
        auto position=Access::next_reference(f,.01);
        // Exercise both correction signs and values above the 3300 envelope.
        for(int i:{0,1,3,4,5}) position[i]+=(n%100<50?50000:-50000);
        Access::tick(f,Access::debug(f).header.stamp.sec+
            Access::debug(f).header.stamp.nanosec*1e-9+.01,position);
        const auto debug=Access::debug(f);
        ASSERT_FALSE(Access::stopped(f));
        for(int i:{0,1,3,4,5}) {
            EXPECT_FLOAT_EQ(debug.limited_pwm[i],std::clamp(debug.raw_pwm[i],-3300.f,3300.f));
            clipped=clipped || std::fabs(debug.raw_pwm[i])>3300;
            negative=negative || debug.limited_pwm[i]<0;
            positive=positive || debug.limited_pwm[i]>0;
        }
        EXPECT_EQ(debug.limited_pwm[2],0);
    }
    EXPECT_TRUE(clipped);EXPECT_TRUE(negative);EXPECT_TRUE(positive);
    Access::begin_stop(f);
    auto debug=Access::debug(f);
    Access::tick(f,debug.header.stamp.sec+debug.header.stamp.nanosec*1e-9+2.1,debug.actual_position);
    EXPECT_TRUE(Access::fully_stopped(f));EXPECT_FALSE(Access::stopped(f));
}

TEST_F(TripodMultiLegTest, OptionalSlewOffStillStopsOnInvalidFeedbackAndEstop) {
    for(bool invalid:{false,true}) {
        warning_fixture();
        auto document=YAML::LoadFile(path);
        document["parameters"]["rinbo_tripod_rslip"]["safety"]["enable_pwm_slew_limit"]=false;
        {std::ofstream out(path);out<<document;}
        {rinbo_config::MotionSession c(rinbo_config::Stage::Calibration,path);c.complete();}
        {rinbo_config::MotionSession s(rinbo_config::Stage::Standing,path);s.complete();}
        rinbo_config::MotionSession session(rinbo_config::Stage::Tripod,path);PIDController f(session);
        if(invalid) EXPECT_TRUE(Access::guard(f,10,1,std::numeric_limits<float>::quiet_NaN()));
        else Access::estop(f);
        EXPECT_TRUE(f.failed());
        for(float pwm:Access::debug(f).limited_pwm) EXPECT_EQ(pwm,0);
    }
}
