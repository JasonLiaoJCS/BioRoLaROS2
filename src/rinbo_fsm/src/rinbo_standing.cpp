...
public:
    explicit StandingController(rinbo_config::MotionSession& session)
        ...
        rotate_timeout_s_ = this->declare_parameter<double>("safety.rotate_timeout_s", 20.0);
...
private:
    ...
    void handle_standing_sample(const std::array<float, 6>& positions,
        ...
        for (int i = 0; i < 6; i++) {
            ...
            switch (leg_states_[i]) {
                ...
                case LegState::ROTATE_180: {
                    ...
                    const double rotate_duration = rinbo_fsm::standing_duration(
                        rotate_180_counts_, target_vel_counts_);
                    const auto reference = rinbo_fsm::standing_reference(
                        rotate_elapsed, rotate_180_counts_, target_vel_counts_);
                    const float target_pos = hall_positions_[i] +
                        (is_left_leg(i) ? -reference.position : reference.position);
                    const float target_vel = reference.velocity;
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 500,
            "STANDING TRACE (raw counts, normalized velocity/PWM): %s",
            rinbo_fsm::tracking_trace(positions, trace_targets, velocities, cmd).c_str());
    ...
    std::array<double, 6> stable_since_s_{{-1,-1,-1,-1,-1,-1}};
