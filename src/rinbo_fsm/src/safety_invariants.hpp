...
// Tripod operator-selected raw FPGA command ceiling; Calibration/Standing/Manual have a separate 500 ceiling.
...
// Existing manual-motion shaping; Tripod's optional rate is independent.
...
inline void validate_pwm(double max_pwm, const std::string& owner) {
    if (!std::isfinite(max_pwm) || max_pwm <= 0.0 || max_pwm > kHardMaxPwm) {
        throw std::invalid_argument(owner + " max_pwm must be finite and in (0,500] raw FPGA command units");
    }
}

inline void validate_tripod_pwm(double max_pwm) {
    if (!std::isfinite(max_pwm) || max_pwm <= 0.0 || max_pwm > kHardMaxTripodPwm) {
        throw std::invalid_argument("Tripod max_pwm must be finite and in (0,3300] raw FPGA command units");
    }
}
...
inline void validate_tripod_motion_safety_contract(
    ...
    // The operator may select warning-only finite tracking errors for attended
    // tuning. Invalid feedback is always rejected by the controller, separately.
    // PWM slew is optional command shaping, not a power/feedback interlock.
    // A low rate can delay braking and even retain the opposite signed command.
    ...
    if (!std::isfinite(safety.max_position_error_counts) ||
        ...
        safety.pwm_slew_rate_per_second > std::numeric_limits<float>::max() ||
        ...
        throw std::invalid_argument(
            ...
            "a finite positive optional PWM slew rate, and position-error samples<=10");
    }
}
