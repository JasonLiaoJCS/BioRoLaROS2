...
public:
    explicit PIDController(rinbo_config::MotionSession& session)
        ...
        max_pwm_ = static_cast<float>(this->declare_parameter<double>("max_pwm", 3300.0));
        ...
        rinbo_fsm::validate_tripod_pwm(max_pwm_);
        ...
        motor_state_guard_ = std::make_unique<rinbo_fsm::RosInputGuard>(
            *this, "/motor/state");
        safety_recording_ = std::make_unique<rinbo_fsm::SafetyRecording>(*this, disabled_legs_.mask(), false);
...
private:
    ...
    void report_reference_budget() {
        ...
        if (!std::isfinite(peak_pwm) || !std::isfinite(peak_slew) ||
            peak_pwm >= max_pwm_ || (safety_.enable_pwm_slew_limit &&
            ...
            reason << "TRIPOD_REFERENCE_WARNING nominal_feedforward_peak=" << peak_pwm
                   ...
                   << " slew_enabled=" << safety_.enable_pwm_slew_limit
        ...
        RCLCPP_INFO(get_logger(),
            "TRIPOD_REFERENCE_BUDGET startup=%.3fs ratios=%.3f->%.3f ff_peak=%.3f/%.3f ff_slew=%.3f/%.3f slew_enabled=%s (model only)",
            startup_duration_, start_ratio_, target_ratio_, peak_pwm, max_pwm_,
            peak_slew, safety_.pwm_slew_rate_per_sec,
            safety_.enable_pwm_slew_limit ? "true" : "false");
    }
    ...
    void load_safety_parameters() {
        ...
        safety_.enable_pwm_slew_limit = this->declare_parameter<bool>("safety.enable_pwm_slew_limit", false);
    ...
    uint32_t publish_motor_command(
        ...
        if (state_ == State::SAFETY_STOP || !motor_handshake_->ready_for_output()) {
    ...
    void publish_safety_event(rclcpp::Time now_time, const std::string& reason) {
        ...
        if(safety_recording_) safety_recording_->emit(reason, first_fault_phase_, tau_, current_ratio_, event.cycle_count, event.header.seq);
    ...
    bool trigger_safety_stop(const std::string& reason) {
        ...
        if(first_failure) first_fault_phase_=state_name(state_);
