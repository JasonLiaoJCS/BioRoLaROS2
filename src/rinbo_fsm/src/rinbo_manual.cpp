...
private:
    ...
    void motor_callback(const rinbo_msgs::msg::MotorStateStamped& msg, const rclcpp::MessageInfo& info) {
        ...
        for (size_t i = 0; i < 6; ++i) {
            if (disabled_.contains(i) || plan_.legs[i].mode == rinbo_manual::Mode::Off) continue;
            ...
            position_[i] = q;
            if (trajectory_) {
                motion_min_[i] = std::min(motion_min_[i], q);
                motion_max_[i] = std::max(motion_max_[i], q);
            }
        }
    ...
    std::string tracking_failure(size_t i, const rinbo_manual::Sample& target) const {
        std::ostringstream out;
        out << (target.done ? "final pose not reached: " : "alignment not reached: ")
            << rinbo_manual::names[i] << std::fixed << std::setprecision(2)
            << "; target_deg=" << target.position[i] << " actual_deg=" << position_[i]
            << " velocity_deg_s=" << velocity_[i] << " pwm=" << last_pwm_[i]
            << " cap=" << cap_ << " encoder_span_deg=" << motion_max_[i]-motion_min_[i]
            << " peak_pwm=" << peak_pwm_[i];
        return out.str();
    }
    void report(double elapsed, const rinbo_manual::Sample& target) {
        ...
        for (size_t i = 0; i < 6; ++i) {
            ...
            else out << "實際=" << position_[i] << "deg " << velocity_[i] << "deg/s"
                     << " 目標=" << target.position[i] << "deg " << target.velocity[i] << "deg/s"
                     << " PWM命令=" << last_pwm_[i] << "/" << cap_;
        }
    ...
    void tick() {
        ...
        if (!trajectory_) {
            ...
            motion_min_ = motion_max_ = position_;
            motion_start_ = t;
        ...
        if ((!alignment_checked_ && elapsed >= trajectory_->alignment_seconds()) || target.done) {
            for (size_t i = 0; i < 6; ++i) {
                ...
                if (std::abs(target.position[i]-position_[i]) > 2.0 || std::abs(velocity_[i]) > 5.0) {
                    stop(tracking_failure(i, target), true); return;
                }
            }
        ...
        std::array<double, 6> requested_pwm{};
        for (size_t i = 0; i < 6; ++i) {
            ...
            const double raw = effort_.command(error,
                target.velocity[i]*rinbo_manual::counts_per_degree,
                control_velocity_[i]*rinbo_manual::counts_per_degree);
            requested_pwm[i] = raw;
            const double limited = std::clamp(raw, -cap_, cap_);
            pwm[i] = std::clamp(limited, last_pwm_[i]-rinbo_fsm::kHardMaxPwmSlewRatePerS*dt,
                                last_pwm_[i]+rinbo_fsm::kHardMaxPwmSlewRatePerS*dt);
        }
        ...
        if (stopping_) return;
        last_pwm_ = pwm;
        for (size_t i = 0; i < 6; ++i) peak_pwm_[i] = std::max(peak_pwm_[i], std::abs(pwm[i]));
        // 20 Hz file trace captures stop/start cycles hidden by the 2 Hz UI.
        // It records commands, never represents them as measured torque.
        if (seconds(t, last_trace_) >= 0.05) {
            last_trace_ = t;
            for (size_t i = 0; i < 6; ++i) {
                if (plan_.legs[i].mode == rinbo_manual::Mode::Off || disabled_.contains(i)) continue;
                RCLCPP_INFO(get_logger(),
                    "MANUAL TRACE time_s=%.3f leg=%s position_deg=%.3f target_deg=%.3f raw_velocity_deg_s=%.3f control_velocity_deg_s=%.3f target_velocity_deg_s=%.3f requested_pwm=%.3f output_pwm=%.3f cap=%.1f",
                    elapsed, rinbo_manual::names[i], position_[i], target.position[i], velocity_[i],
                    control_velocity_[i], target.velocity[i], requested_pwm[i], pwm[i], cap_);
            }
        }
    ...
    std::array<double, 6> control_velocity_{};
    std::array<rinbo_fsm::ControlVelocity, 6> velocity_filter_{};
    Clock::time_point last_trace_{};
    std::array<double, 6> motion_min_{}, motion_max_{}, peak_pwm_{};
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        rinbo_config::MotionSession session(rinbo_config::Stage::Manual,
            rinbo_config::kConfigPath, rinbo_manual::selected_legs(plan));
