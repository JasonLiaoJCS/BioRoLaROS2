...
inline std::vector<std::string> selected_legs(const Plan& plan) {
    std::vector<std::string> result;
    for (size_t i = 0; i < names.size(); ++i)
        if (plan.legs[i].mode != Mode::Off) result.emplace_back(names[i]);
    return result;
}
...
inline Plan parse(const std::string& contents) {
    ...
    if (root["max_pwm"]) p.max_pwm = number(root["max_pwm"], 1.0, 500.0, "max_pwm");
...
inline void set_outputs(Command& cmd, const Plan& p, const std::array<bool, 6>& disabled,
    ...
    for (size_t i = 0; i < 6; ++i) {
        ...
        if (!std::isfinite(pwm[i]) || !std::isfinite(cap) || cap <= 0 || cap > 500)
