# Windows R-Slip v6.6 參考

[回到入口](../README.md)｜[Orin 腿部管理](../orin_multileg_operation.md)

本頁保留提供的 Windows 啟動器流程。文中的 `.cmd`、`.ps1`、`app/` 是 Windows 套件內的檔案，不在這個 ROS 工作區；Jetson 日常單腳測試請用[操作台教學](../manual_leg_control_zh_TW.md)。
...
本次 v6.6 將 Windows 局部重試流程擴充到 Standing、Tripod，沿用現場既有 Orin 程式與設定。Windows 不傳入 FSM 設定檔；多腿屏蔽功能以 Orin 實際部署版本為準。目前 Orin 腿部名單的管理方式請看上方「Orin 腿部管理」。
...
本工作區三個 FSM 自行讀取 **Orin 唯一的有效設定**，由 Orin 決定多腿屏蔽名單、Calibration 是否完成，以及 Standing／Tripod 是否允許。Windows 只負責啟動程序與顯示 Orin 的結果，不替未校正的腿判定成功。
...
本工作區的 Power tool 與 FSM 已共用 Orin 名單；Windows 不傳腿名，也不另建一套設定。實際管理指令見[腿部啟用／屏蔽](../orin_multileg_operation.md)。
...
以下對應本工作區自動載入現場設定的 FSM。先在 Orin 確認有效屏蔿名單，每次只執行一個；不從 Windows 傳入設定檔：
