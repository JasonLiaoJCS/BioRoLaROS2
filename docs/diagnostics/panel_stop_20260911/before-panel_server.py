    ...
    def __init__(self, root, factory=Native):
        ...
        for rid, request, response in self.db.execute('SELECT * FROM requests').fetchall():
            ...
            if old['status'] in ('started', 'running', 'holding', 'native_done', 'accepted'):
                self.finish(rid, 'state_unknown', 31, 'controller_restarted: no automatic replay', None)
                self.set_latch(True)
        ...
        with self.lock:
            if req['action'] == 'ReadLogs':
                return result(req, 'completed', controller_epoch=self.epoch,
                              events=list(self.events), operations=self.operations())
            ...
            elif (self.urgent_pending and not (action == 'EmergencyStop' and self.urgent_kind == 'StopMotion')) or (self.active and action not in URGENT):
    ...
    def work(self):
        while not self.shutdown.is_set():
            ...
            try:
                ...
                self.finish(rid, 'failed', getattr(exc, 'code', 20), str(exc), getattr(exc, 'data', None))
                self.emit('GUI', str(exc))
            finally:
                with self.lock:
