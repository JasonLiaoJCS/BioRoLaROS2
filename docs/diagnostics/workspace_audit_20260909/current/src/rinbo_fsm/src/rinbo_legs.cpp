...
int main(int argc, char** argv) {
    try {
        if (argc < 2) throw std::runtime_error(
            "Usage: rinbo_legs status [--json] | set LEG... | disable LEG... | enable LEG... | enable-all | tripod-position-policy warn|stop [--dry-run] | tune-tripod [--dry-run] KEY=VALUE... | tune-limits calibration|standing|tripod [--dry-run] KEY=VALUE... | tune-motion KP KD K_FF FRICTION_PWM FADE_COUNTS_S");
        const std::string operation = argv[1];
        if (operation == "assert-idle") {
            if (argc != 2) throw std::runtime_error("Usage: rinbo_legs assert-idle");
            rinbo_config::PathLock lock(std::string(rinbo_config::kConfigPath) + ".lock", true);
        ...
        if (operation == "tune-limits") {
            if (argc < 4) throw std::runtime_error("Usage: rinbo_legs tune-limits calibration|standing|tripod [--dry-run] [--expect-revision N] KEY=VALUE...");
            bool preview = false; int64_t revision = -1;
            std::map<std::string, double> updates;
            for (int i = 3; i < argc; ++i) {
                const std::string token = argv[i];
                if (token == "--dry-run") { preview = true; continue; }
                if (token == "--expect-revision") {
                    if (++i >= argc) throw std::runtime_error("Missing revision");
                    size_t used = 0; revision = std::stoll(argv[i], &used);
                    if (used != std::string(argv[i]).size() || revision < 0) throw std::runtime_error("Invalid revision");
                    continue;
                }
                ...
                size_t used = 0; const auto value = std::stod(value_text, &used);
                if (used != value_text.size() || !updates.emplace(key,value).second)
                    throw std::runtime_error("Invalid or duplicate motion limit: " + token);
            }
            std::cout << rinbo_config::tune_limits(rinbo_config::kConfigPath, argv[2], updates, preview, revision) << '\n';
            return 0;
        }
