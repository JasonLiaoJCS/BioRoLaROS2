...
def test_query_timeout_is_recoverable_and_explained(tmp_path, monkeypatch):
    h = Runtime(tmp_path)
    h.command = lambda *args: ['fake-status']
    def timeout(*args, **kwargs):
        raise subprocess.TimeoutExpired(args[0], 8)
    monkeypatch.setattr(subprocess, 'run', timeout)
    with pytest.raises(RuntimeError, match='rinbo_legs 檢查等待逾時'):
        h.query('rinbo_fsm', 'rinbo_legs', 'status', '--json')
...
def test_exit_powers_off_observed_bridge_even_when_not_started_by_panel(tmp_path):
    h=Harness(tmp_path)
    h.status=lambda:dict(bridge_count=1)
    assert not h.power_touched and h.bridge is None and h.remote is None
    assert h.close()
    assert h.events==['off']


def test_failed_exit_status_read_does_not_claim_confirmed_shutdown(tmp_path):
    h=Harness(tmp_path)
    def fail(): raise RuntimeError('status unavailable')
    h.status=fail
    assert not h.close()


def test_calibration_receives_the_actual_selected_plan(tmp_path):
    h=Harness(tmp_path)
    run=h._run
    calls=[]
    def record(package,name,*args,**kwargs):
        if name=='rinbo_cali': calls.append(args)
        return run(package,name,*args,**kwargs)
    h._run=record
    h.execute('selected-plan.yaml','current',True)
    assert calls==[('--plan','selected-plan.yaml')]


def test_calibration_wrapper_waits_for_configured_stages_not_fixed_sixty_seconds(tmp_path):
    from rinbo_control.runtime import calibration_wait_seconds
    site = dict(hash='current', disabled_legs=['L1'], parameters={'rinbo_cali':{
        'safety.servo_homing_timeout_s':40,
        'safety.hall_search_timeout_s':50,
        'safety.stop_timeout_s':10}})
    assert calibration_wait_seconds(site) == 125
    h = Harness(tmp_path)
    h.site = lambda: site
    run = h._run
    waits = []
    def capture(package, name, *args, **kwargs):
        if name == 'rinbo_cali': waits.append(kwargs['timeout'])
        return run(package, name, *args, **kwargs)
    h._run = capture
    h.execute('selected-plan.yaml', 'current', True)
    assert waits == [125]
    site['parameters']['rinbo_cali']['safety.stop_timeout_s'] = float('nan')
    with pytest.raises(ValueError): calibration_wait_seconds(site)


def test_calibration_wait_accepts_the_configured_600_second_ui_range():
    from rinbo_control.runtime import calibration_wait_seconds
    site={'parameters':{'rinbo_cali':{
        'safety.servo_homing_timeout_s':600,'safety.hall_search_timeout_s':600,
        'safety.stop_timeout_s':600}}}
    assert calibration_wait_seconds(site)==2415


@pytest.mark.parametrize('fresh', [True,False])
def test_connection_reuse_requires_fresh_feedback_and_same_target(tmp_path,monkeypatch,fresh):
    h=Runtime(tmp_path,progress=lambda _:None)
    target=('192.168.30.254',50051,'192.168.30.8')
    h.connected_target=target
    h.idle=lambda:None
    h.status=lambda:dict(bridge_count=1,motor_fresh=fresh,power_fresh=fresh)
    h.feedback=SimpleNamespace(bridge_ip=lambda:target[0])
    monkeypatch.setattr('rinbo_control.runtime.route_source',lambda _:target[2])
    calls=[]
    h.connect=lambda *args:calls.append(args)
    h.ensure_connected(*target)
    assert calls==([] if fresh else [target])
    calls.clear()
    h.ensure_connected(target[0],50052,target[2])
    assert calls==[(target[0],50052,target[2])]


def test_busy_controller_rejected_before_ssh_or_startup(tmp_path):
    h = Runtime(tmp_path)
    def busy(): raise RuntimeError('other controller still publishing')
    def forbidden(*args): raise AssertionError('must not connect while busy')
    h.idle = busy
    h.connect = forbidden
    with pytest.raises(RuntimeError, match='other controller'):
        h.ensure_connected('192.168.30.254')


def test_busy_action_does_not_power_off_another_controller(tmp_path):
    h = Harness(tmp_path)
    h.power_touched = True  # Sensors retained by our previous completed action.
    def busy(): raise RuntimeError('other controller still publishing')
    h.idle = busy
    with pytest.raises(RuntimeError, match='other controller'):
        h.execute('plan', 'current', True)
...
def test_calibration_handoff_requires_clean_exit_even_after_done(tmp_path, late_failure):
    script = tmp_path/'calibration_child.py'
    script.write_text(
        "import signal,time\n"
        "def stop(*args):\n"
        + (" print('CALIBRATION SAFETY STOP: real failure after DONE',flush=True)\n raise SystemExit(1)\n"
           if late_failure else " print('CALIBRATION HANDOFF READY',flush=True)\n raise SystemExit(0)\n")
        + "signal.signal(signal.SIGINT,stop)\nprint('State: DONE',flush=True)\nwhile True: time.sleep(.01)\n")
    runtime = Runtime(tmp_path, progress=lambda _:None)
    runtime.command = lambda *args: [sys.executable, str(script)]
    if late_failure:
        with pytest.raises(RuntimeError, match='real failure after DONE'):
            runtime._run('rinbo_fsm', 'rinbo_cali', until_calibrated=True, timeout=3)
    else:
        runtime._run('rinbo_fsm', 'rinbo_cali', until_calibrated=True, timeout=3)
...
def test_receipt_alone_cannot_reuse_calibration_after_sensor_gap(tmp_path):
    ...
    assert '中斷' in runtime.calibration_reason


def test_calibration_reason_distinguishes_power_coverage_and_reuse(tmp_path):
    h = Runtime(tmp_path)
    h.feedback = SimpleNamespace(sensor_epoch=2)
    status = dict(sensors_on=False, motor_fresh=True, power_fresh=True)
    h.status = lambda: status
    assert h.needs_calibration('plan') and '供電' in h.calibration_reason
    status['sensors_on'] = True
    assert h.needs_calibration('plan') and '尚未建立' in h.calibration_reason
    h.calibration_epoch = 2
    h.query = lambda *args: SimpleNamespace(returncode=1, stdout='', stderr='calibration does not cover R2')
    assert h.needs_calibration('plan') and 'R2' in h.calibration_reason
    h.query = lambda *args: SimpleNamespace(returncode=0, stdout='CALIBRATION READY', stderr='')
    assert not h.needs_calibration('plan') and '不重新尋零' in h.calibration_reason
    status['power_fresh'] = False
    assert h.needs_calibration('plan') and '新回讀' in h.calibration_reason
