# 監控與錄製：看回讀、查校正錯誤、保存資料

[回到入口](README.md)｜[單腳測試](manual_leg_control_zh_TW.md)｜[腿部管理](orin_multileg_operation.md)

Orin 執行網頁服務，Windows 瀏覽器查看六腿命令及感測器。面板只有讀取功能，可以先開著，再依原有 Windows 流程執行 Calibration。

## 開啟

需已安裝 `rinbo_ros_bridge`、`rinbo_fsm`、`rinbo_monitor`。若更新了 Bridge 程式，先停止動作，再以原操作介面重新啟動 Bridge，才會使用新版本。
...
1. 用捷徑啟動面板並開啟網頁。若只更新監控程式，重開監控面板並使用新 token 連結即可。
...
每次最多錄製 30 分鐘或約 64 MiB，達限制自動停止並顯示原因。寫入佇列有上限；若磁碟太慢會停止錄製、標示缺漏，而不阻塞 ROS callback。檔案寫入期間副檔名為 `.json.partial`，正常停止後才改為 `.json`；程式正常退出會嘗試完成存檔，斷電或強制殺程序可能留下不完整的 `.partial`。
...
1. 哪隻腳先報錯？是否有 enable 與 PWM，但位置不變？
2. Hall 是否從 1 變 0？收到與轉送的 reset 事件是否增加？位置之後是否接近 0？
3. 是否先出現電壓下降、電流異常、資料過期或 Bridge 安全停止？

面板的「轉送」不代表 sbRIO 已執行；reset 請求也不等於已歸零。回讀不追蹤實際運動時，依紀錄檢查編碼器、接線與回傳通道，不要直接增加出力。

## 要保存更完整的命令與回讀

監控面板錄製的是每秒 10 次的快照，可能漏掉很短的方向切換。需要分析這類問題，可在另一個 Orin 終端機先啟動原始資料記錄：
...
source install/setup.bash
export ROS_DOMAIN_ID=99
ros2 launch rinbo_data_recorder record_experiment.launch.py run_name:=standing_check
```

看到 `Recording...` 後，再依原本流程執行動作。這個指令只收資料，不開 Bridge、不上電、不啟動馬達。動作結束後，在**記錄的終端機**按 Ctrl+C 完成存檔；它不會停止另一個終端機的馬達程式。

檔案存到 `~/rinbo_logs/日期時間_standing_check/`。新版 `raw_bag` 預設包含控制器命令、Bridge 收到／轉送的命令、encoder／Hall、電源及 ROS 日誌，可依時間核對。`summary.csv` 是定時彙整，motor command 欄仍是控制器命令；要確認 Bridge 轉送內容請分析 `raw_bag`。rosbag 仍可能因傳輸或負載漏包，沒有硬體執行確認。
...
| 位置 | 用途 |
|---|---|
| [start_rinbo_monitor.sh](../start_rinbo_monitor.sh) | 啟動腳本 |
| [server.py](../src/rinbo_monitor/rinbo_monitor/server.py) | ROS 回讀與網頁服務 |
| [model.py](../src/rinbo_monitor/rinbo_monitor/model.py) | 畫面資料整理 |
| [recording.py](../src/rinbo_monitor/rinbo_monitor/recording.py) | 開始／停止錄製與 JSON 存檔 |
| [test/](../src/rinbo_monitor/test) | 面板與錄製測試 |
